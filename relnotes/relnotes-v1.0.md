...
- **Binary Package Support**:
  ...
  **pkgsrc-core**, simplifying upgrades and preventing issues with
  source-based updates.  Install them using pkgadd(1).  See the
  [Download](#download) section for the archive.

- **PAX & CAPS in pkgutils**:
  **pkgutils** and **pkgmk** have been updated to optionally handle
  ACLs (acl(5)) and extended attributes (xattr(7)).
  ...
  **pkgmk** using `pax` as the default archive format.  This allows
  ...
  steps (like *post-install* scripts with *setcap*).  See pkgmk(8) and
  pkgadd(8) for details
...
- **dumb_runtime_dir PAM Module**:
  Zeppe-Lin v1.0 now uses the **dumb-runtime-dir** PAM module by
  default to manage `XDG_RUNTIME_DIR`.  See `pam_dumb_runtime_dir(8)`
  for details.
...
- **Flexible Init with Device Manager Choice**:
  You can now choose your device manager in */etc/rc.conf* using the
  `DEVMGR` variable.  `udevd` is used if you don't set it.  See
...
- **Removed Packages**:
  ...
  - xorg-libdmx ([see why](https://github.com/zeppe-lin/pkgsrc-xorg/issues/2))
  - vulkan-tools
  - xorg-libxxf86misc ([see why](https://github.com/zeppe-lin/pkgsrc-xorg/issues/1))
